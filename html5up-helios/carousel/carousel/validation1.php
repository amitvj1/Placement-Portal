...
define("HASH_PBKDF2_INDEX", 3);
    ...
    $ins=mysql_query("SELECT password FROM validate_signup_page WHERE username='$name'");
    ...
    if($row = mysql_fetch_assoc($ins))
    {
        $storedhash=$row['password'];}
                ...
                $checker=validate_password($name1,$storedhash);
                if($checker){
                    $_SESSION['username']=$name;
                    //$_SESSION['password']=$name1;
                ...
                {
                    echo "<script type='text/javascript>alert('doesnot match!');</script>";
